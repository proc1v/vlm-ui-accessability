#!/bin/bash
# LoRA fine-tuning of Qwen3-VL-8B-Instruct on Screen2AX silver data.
#
...
# Hardware: 2x NVIDIA B200 (Blackwell). Requires PyTorch cu128.
...
# B200s have plenty of VRAM — prefer bigger microbatch over more accumulation.
...
# Cap at 1344x32x32 ~= 1.38M pixels so a ~3000x2000 Retina screenshot is
# resized but still has enough detail for small UI elements.
IMG_MIN_PIXELS=$((256 * 32 * 32))
IMG_MAX_PIXELS=$((1344 * 32 * 32))

OUTPUT_DIR=/workspace/trained_models/qwen3vl_8b_screen2ax_lora

deepspeed --include localhost:1,2 --master_port 29500 src/train/train_sft.py \
