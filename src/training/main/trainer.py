...
def train(
    ...
    if args.global_rank == 0 and not args.debug:
        if not getattr(args, '_wandb_watch_done', False):
            wandb.watch(model, log="gradients", log_freq=100)
            args._wandb_watch_done = True
    ...
    for local_step in range(args.steps_per_epoch):
        ...
        epoch_time.update(batch_sec * args.steps_per_epoch / 3600)
        remain_time.update(batch_sec * (args.steps_per_epoch - local_step - 1) / 3600)
        end = time.time()
