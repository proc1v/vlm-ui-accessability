...
from main.eval_screen2ax import validate_screen2ax
...
def parse_args(args):
    ...
    parser.add_argument("--eval_f1_train", action="store_true", default=False, help="evaluate F1 on a train subset each epoch to monitor overfitting")
...
def main(args):
    ...
    elif "screen2ax_tree" in args.val_dataset:
        from main.eval_screen2ax import validate_screen2ax_tree
        validate = validate_screen2ax_tree
    elif "screen2ax" in args.val_dataset:
        validate = validate_screen2ax
    else:
    ...
    # Build train DataLoader manually so DeepSpeed doesn't create one internally.
    # Passing training_data to deepspeed.initialize() causes it to spawn worker
    # processes that try to fork the processor, which deadlocks with --random_sample.
    train_sampler = torch.utils.data.distributed.DistributedSampler(
        train_dataset, shuffle=args.random_sample, drop_last=True
    ) if args.distributed else None
    train_loader = torch.utils.data.DataLoader(
        train_dataset,
        batch_size=args.batch_size,
        shuffle=(train_sampler is None and args.random_sample),
    ...
    # Small fixed subset of train data for overfitting check (same size as val)
    train_eval_loader = None
    if "screen2ax" in args.val_dataset:
        train_eval_size = len(val_dataset) if val_dataset is not None else 200
        train_eval_subset = torch.utils.data.Subset(
            train_dataset,
            list(range(min(train_eval_size, len(train_dataset))))
        )
        train_eval_sampler = torch.utils.data.distributed.DistributedSampler(
            train_eval_subset, shuffle=False, drop_last=False
        ) if args.distributed else None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.no_eval == False and args.eval_f1_train and train_eval_loader is not None and "screen2ax" in args.val_dataset:
            validate_screen2ax(train_eval_loader, model_engine, processor, epoch, global_step, writer, args, split="train_eval")
            if args.distributed:
                dist.barrier()
            model_engine.train()
        ...
        if args.no_eval == False and val_loader is not None:
            if "screen2ax" in args.val_dataset:
                # distributed validation: all ranks participate, results are all_reduced inside
                ...
                if args.global_rank == 0:
                    ...
                    score = 0.0
                if args.distributed:
                    score_tensor = torch.tensor(score, device=f'cuda:{args.local_rank}')
        ...
        # Additionally save best checkpoint
        if args.no_eval or is_best:
            best_dir = os.path.join(args.log_dir, "ckpt_model_best")
            if args.global_rank == 0:
            ...
            try:
                ...
                print("Failed to save best checkpoint (): ", e)
